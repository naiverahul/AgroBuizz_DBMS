...
export interface IStorage {
  ...
  createProductComplaint(
    complaint: InsertProductComplaint
  ...
  updateComplaintStatus(
    complaintId: number,
    status: string
  ): Promise<ProductComplaint | undefined>;
  addVendorResponse(
    complaintId: number,
    response: string
  ): Promise<ProductComplaint | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByUsername(username: string): Promise<User | undefined> {
    try {
      ...
      console.log(
        `[DATABASE] User with username "${username}" found: ${
          user ? "Yes" : "No"
        }`
      );
      return user;
      ...
      console.error(
        `[DATABASE ERROR] Failed to get user by username: ${username}`,
        error
      );
      throw error;
    }
  ...
  async getUserByEmail(email: string): Promise<User | undefined> {
    try {
      ...
      console.log(
        `[DATABASE] User with email "${email}" found: ${user ? "Yes" : "No"}`
      );
      return user;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to get user by email: ${email}`,
        error
      );
      throw error;
    }
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    try {
      console.log(
        `[DATABASE] Creating new user with username: ${
          insertUser.username
        }, type: ${insertUser.userType || "customer"}`
      );
      ...
      console.error(
        `[DATABASE ERROR] Failed to create user: ${insertUser.username}`,
        error
      );
      throw error;
    }
  ...
  async updateUserType(
    ...
    try {
      console.log(
        `[DATABASE] Updating user type for user ID: ${id} to type: ${userType}`
      );
      ...
      if (!user) {
        console.log(
          `[DATABASE] User with ID ${id} not found, cannot update type`
        );
        return undefined;
      }
      ...
      console.log(
        `[DATABASE] User type update ${updatedUser ? "successful" : "failed"}`
      );
      return updatedUser;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to update user type for ID: ${id}`,
        error
      );
      throw error;
    }
  ...
  async getCropById(cropId: string): Promise<Crop | undefined> {
    try {
      ...
      console.log(
        `[DATABASE] Crop with ID "${cropId}" found: ${crop ? "Yes" : "No"}`
      );
      return crop;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to get crop by ID: ${cropId}`,
        error
      );
      throw error;
    }
  ...
  async getFarmerCrops(farmerId: number): Promise<Crop[]> {
    try {
      ...
      const farmerCropsList = await db
        .select()
        .from(crops)
        .where(eq(crops.farmerId, farmerId));

      console.log(
        `[DATABASE] Retrieved ${farmerCropsList.length} crops for farmer ID: ${farmerId}`
      );
      return farmerCropsList;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to fetch crops for farmer ID: ${farmerId}`,
        error
      );
  ...
  async createCrop(crop: InsertCrop): Promise<Crop> {
    try {
      // const cropType = (crop as any).name || crop.type || "unknown";

      console.log(`[DATABASE] Creating new crop: ${crop.type}`);
      ...
      const [newCrop] = await db
        ...
        .values({
          cropId: crop.cropId,
          name: crop.name,
          type: crop.type, // ✅ Trust what was passed from frontend
      ...
      console.log(
        `[DATABASE] Crop created successfully with ID: ${newCrop.cropId}`
      );
      return newCrop;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to create crop: ${
  ...
  async updateCrop(
    cropId: string,
    cropUpdates: Partial<InsertCrop>
  ): Promise<Crop | undefined> {
    try {
      ...
      console.log(
        `[DATABASE] Crop update ${updatedCrop ? "successful" : "failed"}`
      );
      return updatedCrop;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to update crop with ID: ${cropId}`,
        error
      );
      throw error;
    }
  ...
  async deleteCrop(cropId: string): Promise<boolean> {
    try {
      ...
      if (!cropExists) {
        console.log(
          `[DATABASE] Crop with ID "${cropId}" not found, cannot delete`
        );
        return false;
      }
      ...
      console.log(
        `[DATABASE] Crop deletion ${success ? "successful" : "failed"}`
      );
      return success;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to delete crop with ID: ${cropId}`,
        error
      );
      throw error;
    }
  ...
  async searchCrops(query: string): Promise<Crop[]> {
    try {
      ...
      console.log(
        `[DATABASE] Found ${results.length} crops matching "${query}"`
      );
      return results;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to search crops with query: ${query}`,
        error
      );
      throw error;
    }
  ...
  async getProductById(productId: string): Promise<Product | undefined> {
    try {
      ...
      console.log(
        `[DATABASE] Product with ID "${productId}" found: ${
          product ? "Yes" : "No"
        }`
      );
      return product;
      ...
      console.error(
        `[DATABASE ERROR] Failed to get product by ID: ${productId}`,
        error
      );
      throw error;
    }
  ...
  async searchProducts(query: string): Promise<Product[]> {
    try {
      ...
      console.log(
        `[DATABASE] Found ${results.length} products matching "${query}"`
      );
      return results;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to search products with query: ${query}`,
        error
      );
      throw error;
    }
  ...
  async createProduct(product: InsertProduct): Promise<Product> {
    try {
      ...
      console.log(
        `[DATABASE] Product created successfully with ID: ${newProduct.productId}`
      );
      ...
      if (product.vendorId) {
        try {
          ...
          console.log(
            `[DATABASE] Created vendor-product relationship for vendor: ${product.vendorId}, product: ${newProduct.productId}`
          );
        } catch (relationError) {
          console.error(
            `[DATABASE WARNING] Failed to create vendor-product relationship: ${
  ...
  async getVendorById(vendorId: string): Promise<Vendor | undefined> {
    try {
      ...
      console.log(
        `[DATABASE] Vendor with ID "${vendorId}" found: ${
          vendor ? "Yes" : "No"
        }`
      );
      return vendor;
      ...
      console.error(
        `[DATABASE ERROR] Failed to get vendor by ID: ${vendorId}`,
        error
      );
      throw error;
    }
  ...
  async getVendorProducts(vendorId: string): Promise<Product[]> {
    try {
      ...
      const vendorProductEntries = await db
        .select()
        .from(vendorProducts)
        .where(eq(vendorProducts.vendorId, vendorId));

      console.log(
        `[DATABASE] Found ${vendorProductEntries.length} product-vendor relationships for vendor: ${vendorId}`
      );
      ...
      if (vendorProductEntries.length > 0) {
        ...
        console.log(
          `[DATABASE] Retrieved ${productList.length} products for vendor: ${vendorId}`
        );
        return productList;
      }

      console.log(
        `[DATABASE] No products found for vendor: ${vendorId}, falling back to all products`
      );
      // Fallback: return all products for now
      return await this.getProducts();
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to get vendor products for vendor: ${vendorId}`,
        error
      );
      return []; // Return empty array on error
    }
  ...
  async createProductComplaint(
    ...
    try {
      console.log(
        `[DATABASE] Creating new product complaint for product: ${complaint.productId}, by user: ${complaint.userId}`
      );
      const [newComplaint] = await db
        .insert(productComplaints)
        .values(complaint)
        .returning();

      console.log(
        `[DATABASE] Complaint created successfully with ID: ${newComplaint.id}`
      );
      return newComplaint;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to create product complaint`,
        error
      );
      throw error;
    }
  ...
  async getProductComplaints(userId: number): Promise<ProductComplaint[]> {
    try {
      ...
      console.log(
        `[DATABASE] Retrieved ${complaints.length} complaints for user: ${userId}`
      );
      return complaints;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to fetch complaints for user: ${userId}`,
        error
      );
      throw error;
    }
  ...
  async getVendorComplaints(vendorId: string): Promise<ProductComplaint[]> {
    try {
      ...
      console.log(
        `[DATABASE] Retrieved ${complaints.length} complaints for vendor: ${vendorId}`
      );
      return complaints;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to fetch complaints for vendor: ${vendorId}`,
        error
      );
      throw error;
    }
  ...
  async updateComplaintStatus(
    ...
    try {
      console.log(
        `[DATABASE] Updating complaint ID: ${complaintId} to status: ${status}`
      );
      const [updatedComplaint] = await db
        .update(productComplaints)
        .set({
      ...
      console.log(
        `[DATABASE] Complaint status update ${
          updatedComplaint ? "successful" : "failed"
        }`
      );
      return updatedComplaint;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to update complaint status for ID: ${complaintId}`,
        error
      );
      throw error;
    }
  ...
  async addVendorResponse(
    ...
    try {
      console.log(
        `[DATABASE] Adding vendor response to complaint ID: ${complaintId}`
      );
      const [updatedComplaint] = await db
        .update(productComplaints)
        .set({
      ...
      console.log(
        `[DATABASE] Vendor response added ${
          updatedComplaint ? "successfully" : "failed"
        }`
      );
      return updatedComplaint;
    } catch (error) {
      console.error(
        `[DATABASE ERROR] Failed to add vendor response for complaint ID: ${complaintId}`,
        error
      );
      throw error;
    }
