...
export function setupAuth(app: Express) {
  ...
  app.post("/api/register", async (req, res) => {
    try {
      const existingUser = await db.query.users.findFirst({ where: eq(users.username, req.body.username) });
      if (existingUser) {
        return res.status(400).json({ success: false, message: "Username already exists" });
      }

      const existingEmail = await db.query.users.findFirst({ where: eq(users.email, req.body.email) });
      if (existingEmail) {
      ...
      req.login(user, (err) => {
        if (err) return res.status(500).json({ success: false, message: "Login error after registration" });
